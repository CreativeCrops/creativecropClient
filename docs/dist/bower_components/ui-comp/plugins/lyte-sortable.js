		...
		$L.prototype.sortable = function(object) {
			...
			var mouseMoveEvent = function(event){
				...
				if(_sortableElem){
					...
					if(data && data._isDown){
						...
						if(!_prevScrollDiv || (_prevScrollDiv && !_prevScrollDiv.isEqualNode(scrollDiv))){
						...
						if(window.getComputedStyle(scrollDiv).position == "relative"){
							isRelativeY = true;
						...
						if(data.multiSelect & _multiSelectedItems.length > 1){
							...
							for(var i = 0; i<_multiSelectedItems.length; i++){
								_multiSelectedItems[i].style.display = "";
							}
			...
			var callForScrollX = function(data,parentOffset,_mousePosition,_offset,parent,isRelative){
				...
				if((divOffset.right >= parentOffset.right) && (_scrollLeft <= _maxScrollWidth)){
					if((_maxScrollWidth - _scrollLeft) >= 10){
						data.scrollDivX.scrollLeft += 10;
						if(isRelative){
							data._div.style.left = _mousePosition.x - _offset[0] - parent.left + 10 + _scrollLeft + "px";
						}
					}
					else{
						data.scrollDivX.scrollLeft += (10 - (_maxScrollWidth - _scrollLeft));
						if(isRelative){
							data._div.style.left = _mousePosition.x - _offset[0] - parent.left + (10 - (_maxScrollWidth - _scrollLeft)) + _scrollLeft + "px";
						}
					}
				}
				else if((divOffset.left <= parentOffset.left) && (_scrollLeft > 0)){
					if(isRelative){
						data._div.style.left = _mousePosition.x - _offset[0] - parent.left - ((_maxScrollWidth - _scrollLeft) > 10 ? 10 : (10 - (_maxScrollWidth - _scrollLeft))) + _scrollLeft + "px";
					}
					data.scrollDivX.scrollLeft -= 10;
				}
				else{
			...
			var callForScrollY = function(data,scrollDiv,parentOffset,_maxScrollHeight,topNBottom,isRelativeY,_mousePosition,_offset,parent){
				...
				if(data.multiSelect && topNBottom){
					if((topNBottom.top <= parentOffset.top) && (scrollTop > 0)){
					...
					if((divOffset.top <= parentOffset.top) && (scrollTop > 0)){
						if(isRelativeY){
					...
					else if((divOffset.bottom >= parentOffset.bottom) && (scrollTop <= _maxScrollHeight)){
