	...
	if($L){	
		...
		$L.prototype.draggable = function(object) {
			...
			if(!manageDraggable.bindEvent){
				...
				manageDraggable.bindEvent = true;
			}
			...
			data.cursorAt = data.cursorAt ? data.cursorAt : null;
				...
				if(_handleElement){
					...
					if(data && data._isDown){
						...
						if(!data._isMoved){
							...
							elem.style.width = elemOffset.width +'px';
							elem.style.height = elemOffset.height +'px';
			...
			var onBeforeStop = function(data, _handleElement){
			...
			var onStop = function(data){
				returnVal = data.onStop(data._element, data._positionedPlceholder ? data._element.parentElement : null);
				return (returnVal === undefined) ? true : returnVal;
			}
			/*---------------Callbacks End--------------*/
