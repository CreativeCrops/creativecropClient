...
window.addEventListener("load",function(){
  ...
  for(var key in store.model){
    var pK = store.model[key]._primaryKey;
    models.push({modelName:key,pK:pK});
  }
  var scope = store.adapter.$.getCallBackScope("app", "idbBeforeOpen"), args;
  if(scope){
    args = this.constructArgs(models);
    models = this.callBack(scope, args);
  }
    ...
    IDBInsert:function(){
      ...
      if(!isProcessing && Object.keys(store.$.idbQ2).length){
        ...
        if(module){
            ...
            var pK = store.modelFor(module)._primaryKey;
            if(q.length){
