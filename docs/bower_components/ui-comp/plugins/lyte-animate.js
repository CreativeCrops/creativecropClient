(function(window){
	...
	AnimationData.prototype = {
		add : function(elem){
			...
			this.data[randomId] = [];
			elData.elementProp[elem.lyteAnimateId] = {};
			elem.addEventListener('DOMNodeRemovedFromDocument', function(evt){
				if(this.lyteAnimateId){
					delete elData.elementProp[this.lyteAnimateId]
				}
			})
			return randomId;
		},
	...
	ElementData.prototype = {
		set :  function(){
			...
			if(arguments.length == 3){
				if(this.elementProp[arguments[0].lyteAnimateId] == undefined){
					this.elementProp[arguments[0].lyteAnimateId] = {};
				}
				this.elementProp[arguments[0].lyteAnimateId][arguments[1]] = arguments[2];
			}
			else if(arguments.length == 4){
				if(this.elementProp[arguments[0].lyteAnimateId][arguments[1]] == undefined){
					this.elementProp[arguments[0].lyteAnimateId][arguments[1]] = {};
				}
				this.elementProp[arguments[0].lyteAnimateId][arguments[1]][arguments[2]] = arguments[3]
			}
		...
		get : function(){
			...
			if(arguments.length == 1){
				return this.elementProp[arguments[0].lyteAnimateId];
			}
			else if(arguments.length == 2){
				return this.elementProp[arguments[0].lyteAnimateId][arguments[1]];
			}
			else if(arguments.length == 3){
				if(this.elementProp[arguments[0].lyteAnimateId][arguments[1]]){
					return this.elementProp[arguments[0].lyteAnimateId][arguments[1]][arguments[2]];
				}	
			}
			return;
		}
	...
	function findAnimeProperties (){
			...
			for(var i = 0; i < keys.length; i++){
				...
				if(newObj[kk]){
					if((compStyle[kk] == newObj[kk] || parseInt(compStyle[kk]) == newObj[kk] || newObj[kk] == this.style[kk]) && !hidden){
	...
	function stopAnimation(jumpToEnd, clearQueue){
		if(jumpToEnd){
			...
			for(var i = 0; i < props.length; i++){
				...
				$L(this).css(props[i], this.style[props[i]].replace(/([0-9.-]+)/, function(arg){
	...
	if(lyteNode){
		...
		lyteNode.prototype.fadeTo = function(duration, value, ease, callback){
			if(duration < 0){
				var temp = duration;
				duration = value;
				value = temp;
