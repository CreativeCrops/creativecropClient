	...
	if($L){
		...
		mIManager = {
			...
			htmlEncode : function (str) {
			  ...
			  var src = '(?:' + Object.keys(mIManager.escapeMap).join('|') + ')'; //NO I18n
			  var testRegexp = RegExp(src);
    		  var replaceRegexp = RegExp(src, 'g'); //NO I18n
    		  str = str == null ? '' : '' + str; //NO I18n
		      return testRegexp.test(str) ? str.replace(replaceRegexp, escaper) : str;
		    },
		    mentionsSyntax : function(mention){
		    	return "["+mention.value+"]("+mention.type+":"+mention.id+")"; //NO I18n
		    },
			...
			setCaratPosition : function (domNode, caretPos) {
		      if (domNode.createTextRange) {
		        ...
		        range.move('character', caretPos); //NO I18n
		        range.select();
		      } else {
			...
			init : function(element){
				var ele1 = mIManager.createElement("div",null,"lyteMIWrapper"); //NO I18n
				LyteComponent.insertBefore(element,ele1);
				ele1.append(element);
				var ele = mIManager.createElement("div",null,"lyteMIDropdown"); //NO I18n
				LyteComponent.insertAfter(element,ele);
				mIManager.copyTextarea(element);
				var cs = window.getComputedStyle(element);
				var ele2 = mIManager.createElement("div",null,"lyteMentions"); //NO I18n
				ele2.style.fontFamily = cs.fontFamily;
				ele2.style.fontSize = cs.fontSize;
				ele2.style.fontWeight = cs.fontWeight;
				ele2.append(mIManager.createElement("div",null,null)); //NO I18n
				LyteComponent.insertBefore(element,ele2);
				return ele;
			},
			rtrim : function(string) {
    			return string.replace(/\s+$/,""); //NO I18n
  			},
  			highlightContent : function (value, term) {
			    if (!term && !term.length) {
			      return value;
			    }
			    return value.replace(new RegExp("(?![^&;]+;)(?!<[^<>]*)(" + term + ")(?![^<>]*>)(?![^&;]+;)", "gi"), "<b>$1</b>"); //NO I18n
		    },
		    copyTextarea : function(element) {

		      //	We will create a div clone of the textarea
		      //	by copying these attributes from the textarea to the div.
		      var attrs = [ 'paddingTop', 'paddingRight', 'paddingBottom', 'paddingLeft', 'marginTop', 'marginRight', 'marginBottom', 'marginLeft', 'fontSize', 'lineHeight', 'fontFamily', 'width', 'fontWeight', 'border-top-width', 'border-right-width', 'border-bottom-width', 'border-left-width', 'borderTopStyle', 'borderTopColor', 'borderRightStyle', 'borderRightColor', 'borderBottomStyle', 'borderBottomColor', 'borderLeftStyle', 'borderLeftColor', 'box-sizing', '-moz-box-sizing', '-webkit-box-sizing']; //NO I18n

		        // CopyTextarea only works on textareas
		        if (element.type !== 'textarea') { //NO I18n
		          return false;
		        }

		        var mITextarea = $L(element),
		          cloneTextarea = $L(mIManager.createElement('div')).css({'position': 'absolute','visibility':'hidden','display':'none','word-wrap':'break-word'}), //NO I18n
		          lineHeight = parseInt(mITextarea.css('line-height'), 10) || parseInt(mITextarea.css('font-size'), '10'), //NO I18n
		          minheight = parseInt(mITextarea.css('height'), 10) || lineHeight * 3, //NO I18n
		          maxheight = parseInt(mITextarea.css('max-height'), 10) || Number.MAX_VALUE, //NO I18n
		          goalheight = 0;
		        ...
		        function setCloneDivWidth() {
		          ...
		          if(mITextarea.e._mIData && mITextarea.e._mIData._overlayDiv){
		          	$L(mITextarea.e.previousElementSibling).css("fontSize",mITextarea.css("fontSize")); //NO I18n
		          }
		          if (cloneTextarea.width() !== curatedWidth) {
		            cloneTextarea.css({'width': curatedWidth + 'px', 'fontSize': mITextarea.css("fontSize")}); //NO I18n

		            if(cloneTextarea.e.getAttribute('setHeight') === "true"){ //NO I18n
		            	cloneTextarea.e.setAttribute('setHeight',"false"); //NO I18n
		            }
		            else{
		        ...
		        function setHeightAndOverflow(height, overflow) {
		          ...
		          if (mITextarea.height() !== curratedHeight) {
		            mITextarea.css({'height': curratedHeight + 'px','overflow':overflow}); //NO I18n
		        ...
		        function updateTextarea(forced) {
		          ...
		          var cloneContent = cloneTextarea.html().replace(/<br>/ig, '<br />'); //NO I18n

		          if (forced || textareaContent + '&nbsp;' !== cloneContent) { //NO I18n

		            // Add an extra white space so new rows are added when you are at the end of a row.
		            cloneTextarea.html(textareaContent + '&nbsp;'); //NO I18n

		            // Change textarea height if twin plus the height of one line differs more than 3 pixel from textarea height
		            if (Math.abs(cloneTextarea.outerHeight() + lineHeight - mITextarea.outerHeight()) >= 3) {
		            	cloneTextarea.e.setAttribute('setHeight',"true"); //NO I18n
		              var goalheight = cloneTextarea.outerHeight();
		              if (goalheight >= maxheight) {
		                setHeightAndOverflow(maxheight, 'auto'); //NO I18n
		              } else if (goalheight <= minheight) {
		                setHeightAndOverflow(minheight, 'hidden'); //NO I18n
		              } else {
		                setHeightAndOverflow(goalheight, 'hidden'); //NO I18n
		              }
		        ...
		        while (i--) {
		          ...
		          if (attrs[i].toString() === 'width' && mITextarea.css(attrs[i].toString()) === '0px') { //NO I18n
		            setCloneDivWidth();
		          } else {
		        ...
		        mITextarea.e.addEventListener('input', updateTextarea); //NO I18n
		        mITextarea.e.addEventListener('change', updateTextarea); //NO I18n
		        if(!mIManager.bindResize){
		        	window.addEventListener('resize', callAtTheEndOfResize); //NO I18n
		...
		$L.prototype.mentionsInput = function(object) {
			...
			var modifyNodeContent = function() {
		      ...
		      textarea._mIData.lyteMICollection.forEach(function (mention) {
		        var textSyntax = textarea._mIData.triggerChar+mIManager.mentionsSyntax(mention);
		        var textHighlight = "<strong><span>"+mention.value+"</span></strong>"; //NO I18n

		        mentionText = mentionText.replace(textSyntax, textHighlight);
		      });

		      mentionText = mentionText.replace(/\n/g, '<br />'); //NO I18n
		      mentionText = mentionText.replace(/ {2}/g, '&nbsp; '); //NO I18n
			...
			var updateCollection = function(value, id, type) {
		      ...
		      var regex = new RegExp("\\" + textarea._mIData.triggerChar + mIManager.currentDataQuery, "gi"); //NO I18n
		      ...
		      var updatedMessageText = start + value + end + " "; //NO I18n
		      ...
		      resetBuffer();
		      mIManager.currentDataQuery = ''; //NO I18n
		      hideDropdown();
		      ...
		      textarea.dispatchEvent(new InputEvent('change')); //NO I18n
			...
			var hideDropdown = function(input){
				...
				lyteMIList.style.height = "auto"; //NO I18n
				$L(lyteMIList).removeClass('lyteMIDisplayBlock','lyteMIAddOverflow'); //NO I18n
			};
			...
			var createLiElement = function(data){
				var ele = mIManager.createElement('li'); //NO I18n
				$L(ele).attr('data-display',data[textarea._mIData.searchBy]); //NO I18n
				$L(ele).attr('data-ref-id',data.id + ""); //NO I18n
				$L(ele).attr('data-ref-type',data.type); //NO I18n
			...
			var onSelect = function(event) {
		    	...
		    	if(!lyteMIList){
		    		lyteMIList = this.closest('.lyteMIDropdown'); //NO I18n
		    	}
		    	textarea = lyteMIList._textarea;
		    	updateCollection(elmTarget.attr('data-display'), elmTarget.attr('data-ref-id'), elmTarget.attr('data-ref-type')); //NO I18n
			...
			var onKeyUpOrDown = function(ele){
				...
				if($L('li',lyteMIList).e.length){ //NO I18n
					$L('li',lyteMIList).removeClass(textarea._mIData.activeItem); //NO I18n
				}
			...
			var populateDropdown = function(results){
				if(textarea._mIData.wrapper && !textarea._mIData.listAppended){
					...
					// if(!textarea._mIData.listAppended){
						...
						lyteMIList.style.left = wrapperOffset.left + "px"; //NO I18n
						lyteMIList.style.right = wrapperOffset.right + "px"; //NO I18n
						lyteMIList.style.position = "fixed"; //NO I18n
						lyteMIList.style.width = parseInt(wrapperOffset.width) - 2 + "px"; //NO I18n
						textarea._mIData.listAppended = true;
					// }
				}
				  $L(lyteMIList).addClass('lyteMIDisplayBlock'); //NO I18n
			      ...
			      var elmDropDownList = mIManager.createElement('ul'); //NO I18n
			      hideDropdown();

			      for(var i = 0; i<results.length; i++){
			      	  var liEle = createLiElement(results[i]);
			      	  liEle.addEventListener('mousedown',onSelect,true); //NO I18n
			      ...
			      $L(lyteMIList).addClass('lyteMIHide'); //NO I18n
			      $L(lyteMIList).addClass('lyteMIDisplayBlock'); //NO I18n
			      ...
			      if(winH - listOffset.top < listOffset.height){
			      	...
			      	$L(lyteMIList).addClass('lyteMIAddOverflow'); //NO I18n
			      	lyteMIList.scrollTop = 0;
			      }
			      $L(lyteMIList).removeClass('lyteMIHide'); //NO I18n
			};
  						...
  						if( responseData.then ) {
							Promise.resolve( responseData ).then( function( arg ) {
								if(data.dropbody === false) {
									data.displayMentions(arg, query);
									onKeyUpOrDown(lyteMIList.querySelector('li')); //NO i18n
								} else {
			...
			if(typeof object === "string"){ //NO I18n
				if(object === "setMessage"){ //NO I18n
					textarea =  this.e.length ? (this.e[0].tagName === "LYTE-INPUT" && this.e[0].ltProp('type') == "textarea" ? this.e[0].querySelector('textarea') : this.e[0]) : (this.e.tagName === "LYTE-INPUT" && this.e.ltProp('type') == "textarea" ? this.e.querySelector('textarea') : this.e); //NO I18n
					if(textarea.getAttribute('data-lyte-mentions')){ //NO I18n
						if(typeof arguments[1] === "object"){ //NO I18n
							// if(!lyteMIList){
							...
							collection.forEach(function(mention){
								...
								if(message.indexOf(textSyntax) != -1){
									...
									var textHighlight = "<strong><span>"+mentionObj.value+"</span></strong>"; //NO I18n
							...
							// console.error("Inappropriate arguments passed. Please pass an object containing a message and a collection to be set."); //NO I18n
							return null;
						}
					}
					else{
						// console.error(this + " is not Initialized using lyte-mentions plugin.Please check."); //NO I18n
						return null;
					}
				}

				if(object === "getMessage"){ //NO I18n
					textarea =  this.e.length ? (this.e[0].tagName === "LYTE-INPUT" && this.e[0].ltProp('type') == "textarea" ? this.e[0].querySelector('textarea') : this.e[0]) : (this.e.tagName === "LYTE-INPUT" && this.e.ltProp('type') == "textarea" ? this.e.querySelector('textarea') : this.e); //NO I18n
					if(textarea.getAttribute('data-lyte-mentions')){ //NO I18n
				...
				if(object === "getCollection"){ //NO I18n
					textarea =  this.e.length ? (this.e[0].tagName === "LYTE-INPUT" && this.e[0].ltProp('type') == "textarea" ? this.e[0].querySelector('textarea') : this.e[0]) : (this.e.tagName === "LYTE-INPUT" && this.e.ltProp('type') == "textarea" ? this.e.querySelector('textarea') : this.e); //NO I18n
					if(textarea.getAttribute('data-lyte-mentions')){ //NO I18n
			...
			if(this.e.length){
				...
				for(var i=0; i<elemArray.length; i++){
					if(elemArray[i].tagName == "LYTE-INPUT" && elemArray[i].ltProp('type') == "textarea"){ //NO I18n
						var elem = elemArray[i].querySelector('textarea'); //NO I18n
				...
				if(this.e.tagName == "LYTE-INPUT" && this.e.ltProp('type') == "textarea"){ //NO I18n
					element = this.e.querySelector('textarea'); //NO I18n
			...
			element.setAttribute('data-lyte-mentions',true); //NO I18n
			//Initialize values
			// data._textarea = element;
			 //contains,startsWith, endsWith
			data.mode = data.mode ? data.mode : "contains"; //NO I18n
			data.activeItem = data.activeItem ? data.activeItem : "lyteMIActive"; //NO I18n
			data.triggerChar = data.triggerChar ? data.triggerChar : '@'; //NO I18n
			data.minChars = data.minChars == undefined ? 1 : parseInt(data.minChars);
			data.avatars = data.avatars == undefined ? true : data.avatars;
			data.searchBy = data.searchBy ? data.searchBy : "name"; //NO I18n
			...
			var keydownEvent = function(event){
	            ...
	            if ($L(lyteMIList).css("display") != "block") { //NO I18n
	                return ;
	            }
	            ...
	            switch (event.keyCode) {
	                ...
	                case KEY.TAB:
	                    if (lyteMIActiveItem && lyteMIActiveItem.length) {
				            ...
				            lyteMIActiveItem[0].dispatchEvent(new Event('mousedown')); //NO I18n
			...
			if (!data.isLyteInput) {
				$L(element.previousElementSibling).addClass('lyteMentionsNormalTextarea'); //NO I18n
			}
	    	...
	    	element.addEventListener('keydown', keydownEvent); //NO I18n
	    	element.addEventListener('keypress', keypressEvent); //NO I18n
	    	element.addEventListener('input', inputEvent); //NO I18n
	    	element.addEventListener('click', clickEvent); //NO I18n
	    	element.addEventListener('blur', blurEvent); //NO I18n
	    	/*---------------------------------------------------------*/
